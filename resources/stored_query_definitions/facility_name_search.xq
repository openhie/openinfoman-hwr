...
<CSD xmlns:csd="urn:ihe:iti:csd:2013"  >
  ...
  <facilityDirectory>
	...
	then csd_bl:filter_by_other_id($facs5,$careServicesRequest/requestParams/otherID)      
      ...
      let $facs7:= if (exists($careServicesRequest/requestParams/start)) then
	if (exists($careServicesRequest/requestParams/max)) 
	  then csd_bl:limit_items($facs6,$careServicesRequest/requestParams/start,$careServicesRequest/requestParams/max)         
	else csd_bl:limit_items($facs6,$careServicesRequest/requestParams/start,<max/>)         
      else
	if (exists($careServicesRequest/requestParams/max)) 
	  then csd_bl:limit_items($facs6,<start/>,$careServicesRequest/requestParams/max)         
	else $facs6
