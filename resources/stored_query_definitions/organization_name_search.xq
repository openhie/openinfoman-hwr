...
<CSD xmlns:csd="urn:ihe:iti:csd:2013"  >
  <organizationDirectory>
	...
	then csd_bl:filter_by_other_id($orgs5,$careServicesRequest/requestParams/otherID)
      ...
      let $orgs7 := if (exists($careServicesRequest/requestParams/start)) then
	if (exists($careServicesRequest/requestParams/max))
	  then csd_bl:limit_items($orgs6,$careServicesRequest/requestParams/start,$careServicesRequest/requestParams/max)
	else csd_bl:limit_items($orgs6,$careServicesRequest/requestParams/start,<max/>)
      else
	if (exists($careServicesRequest/requestParams/max))
	  then csd_bl:limit_items($orgs6,<start/>,$careServicesRequest/requestParams/max)
	else $orgs6
