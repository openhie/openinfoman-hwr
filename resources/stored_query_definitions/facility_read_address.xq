...
let $facs0 := if (exists($careServicesRequest/requestParams/address/@type)) then /CSD/facilityDirectory/*  else ()
...
let $facs2 := 
  if (count($facs1) = 1) 
	...
	{(
	  for $address in $facility/address[@type = $careServicesRequest/requestParams/address/@type]
	  return $address
	 )}
      {$facility/record}
    </facility>
