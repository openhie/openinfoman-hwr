...
let $oh := $provs3[1]/facilities/facility[upper-case(@entityID) =upper-case($careServicesRequest/requestParams/facility/@entityID)]/service[position() = $careServicesRequest/requestParams/facility/service/@position]/operatingHours[position() = $careServicesRequest/requestParams/facility/service/operatingHours/@position]
return
  if (count($oh) = 1 and count($provs3) = 1)
    then
    let $new_oh := $careServicesRequest/requestParams/facility/service/operatingHours
    let $provs4 := 
	...
	<facility entityID="{$careServicesRequest/requestParams/facility/@entityID}">
	  <service position="{$careServicesRequest/requestParams/facility/service/@position}" >
	    <operatingHours position="{$careServicesRequest/requestParams/facility/service/operatingHours/@position}"/>
	  </service>
	</facility>
      </facilities>
    </provider>
